...
public class SearchCriteria extends Criteria implements java.io.Serializable, Cloneable {
  ...
  /**
   * @param taxonomy String which represents a list of axis value separated with a ';' as 1:2;3:6
   * @return a list of SearchCriteria built from taxonomy parameter or null if parameter is not
   * right
   */
  public static List<SearchCriteria> getCriterias(String taxonomy) {
    if (StringUtil.isDefined(taxonomy)) {
      List<SearchCriteria> criterias = new ArrayList<SearchCriteria>();
      for (String axisValue : taxonomy.split(";")) {
        try {
          String[] axisValueArray = axisValue.split(":");
          criterias.add(new SearchCriteria(Integer.parseInt(axisValueArray[0]), axisValueArray[1]));
